...
// Resolve finds the latest version of a node's record.
func (t *UDPv4) Resolve(n *enode.Node) *enode.Node {
	fmt.Println("testing Resolve", "test")
	...
	// Otherwise, perform a network lookup using MLDsa87.
	var key enode.MLDsa87Key
	if err := n.Load(&key); err != nil {
		return n // No ML-DSA-87 key
	}

	// Convert `enode.MLDsa87Key` to `mldsa87.PublicKey`
	mldsa87PubKey := (*cryptod.PublicKey)(&key)

	// Convert `mldsa87.PublicKey` to raw bytes
	rawKey := cryptod.FromMLDsa87Pub(mldsa87PubKey)

	// Convert raw bytes back to `mldsa87.PublicKey`
	mlKey, err := cryptod.UnmarshalPubkey(rawKey)
	if err != nil {
		return n
	}

	// Perform a lookup using ML-DSA-87 public key.
	result := t.LookupPubkey(mlKey)
...
// LookupPubkey searches for a node based on its ML-DSA-87 public key.
func (t *UDPv4) LookupPubkey(key *cryptod.PublicKey) []*enode.Node {
	if t.tab.len() == 0 {
	...
	// Convert ML-DSA-87 public key to a suitable format
	encodedKey := cryptod.FromMLDsa87Pub(key)

	// Ensure the key is wrapped correctly as encPubkey
	targetKey := encPubkey(encodedKey)

	return t.newLookup(t.closeCtx, targetKey).run()
}
...
func (t *UDPv4) lookupSelf() []*enode.Node {
	...
	pubKey := t.priv.Public().(*cryptod.PublicKey)

	// Encode ML-DSA-87 public key into Pubkey format
	encodedPubKey := encodePubkey(pubKey)

	// Perform the lookup using the encoded public key
	return t.newLookup(t.closeCtx, encodedPubKey).run()
}
...
func (t *UDPv4) newLookup(ctx context.Context, targetKey encPubkey) *lookup {
	target := enode.ID(cryptod.Keccak512Hash(targetKey[:]))
...
func (t *UDPv4) nodeFromRPC(sender *net.UDPAddr, rn v4wire.Node) (*node, error) {
	...
	// Decode ML-DSA-87 Public Key instead
	mldsaPubKey, err := cryptod.UnmarshalPubkey(rn.ID[:]) // Convert bytes to *mldsa87.PublicKey
	if err != nil {
		return nil, err
	}

	// Use ML-DSA-87 key in enode.NewV4
	n := wrapNode(enode.NewV4(mldsaPubKey, rn.IP, int(rn.TCP), int(rn.UDP)))
	err = n.ValidateComplete()
	return n, err
}

func nodeToRPC(n *node) v4wire.Node {
	fmt.Println("testing nodeToRPC", "test")

	var ekey v4wire.Pubkey

	// Attempt to load the ML-DSA-87 public key
	var mlKey enode.MLDsa87Key
	if err := n.Load(&mlKey); err == nil {
		// Convert `enode.MLDsa87Key` to `mldsa87.PublicKey`
		mldsaPubKey := (*cryptod.PublicKey)(&mlKey)

		// Encode ML-DSA-87 public key directly
		ekey = v4wire.EncodePubkey(mldsaPubKey)
	}

	// Return the constructed node information
	return v4wire.Node{
		ID:  ekey,
		IP:  n.IP(),
		UDP: uint16(n.UDP()),
...
type packetHandlerV4 struct {
	v4wire.Packet
	senderKey *cryptod.PublicKey // used for ping
...
func (t *UDPv4) verifyPing(h *packetHandlerV4, from *net.UDPAddr, fromID enode.ID, fromKey v4wire.Pubkey) error {
	...
	// Decode ML-DSA-87 public key
	senderKey, err := cryptod.UnmarshalPubkey(fromKey[:])
	if err != nil {
		return err
	}
	...
	// Store the sender's key in the handler
	h.senderKey = senderKey
	return nil
}

func (t *UDPv4) handlePing__(h *packetHandlerV4, from *net.UDPAddr, fromID enode.ID, mac []byte) {
	req := h.Packet.(*v4wire.Ping)
...
func (t *UDPv4) handlePing(h *packetHandlerV4, from *net.UDPAddr, fromID enode.ID, mac []byte) {
	...
	// Convert sender key from ML-DSA-87
	mldsaPubKey, err := cryptod.UnmarshalPubkey(cryptod.FromMLDsa87Pub(h.senderKey)) // Convert ML-DSA-87
	if err != nil {
		return // Invalid key, don't proceed
	}

	// Use ML-DSA-87 key in enode.NewV4
	n := wrapNode(enode.NewV4(mldsaPubKey, from.IP, int(req.From.TCP), from.Port))
	if time.Since(t.db.LastPongReceived(n.ID(), from.IP)) > bondExpiration {
		t.sendPing(fromID, from, func() {
			t.tab.addVerifiedNode(n)
		})
...
func (t *UDPv4) handleFindnode(h *packetHandlerV4, from *net.UDPAddr, fromID enode.ID, mac []byte) {
	...
	target := enode.ID(cryptod.Keccak512Hash(req.Target[:]))
