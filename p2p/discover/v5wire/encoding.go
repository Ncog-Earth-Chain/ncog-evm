...
func NewCodec(ln *enode.LocalNode, key *cryptod.PrivateKey, clock mclock.Clock) *Codec {
	c := &Codec{
		sha256:    sha256.New(),
		localnode: ln,
		privkey:   key,
...
func (c *Codec) makeHandshakeAuth(toID enode.ID, addr string, challenge *Whoareyou) (*handshakeAuthData, *session, error) {
	fmt.Println("testing makeHandshakeAuth", "test")
	...
	var remotePubkey = new(cryptod.PublicKey) // Using MLDsa87 public key
	if err := challenge.Node.Load((*enode.MLDsa87Key)(remotePubkey)); err != nil {
		return nil, nil, fmt.Errorf("can't find MLDsa87 key for recipient")
	}
	...
	// Extract Public Key from Ephemeral Key
	ephPubKey, ok := ephkey.Public().(*cryptod.PublicKey)
	if !ok {
		return nil, nil, fmt.Errorf("failed to derive ephemeral public key")
	}

	// Convert MLDsa87 Public Key to Bytes
	ephpubkey := cryptod.FromMLDsa87Pub(ephPubKey)
	auth.pubkey = ephpubkey[:]
	auth.h.PubkeySize = byte(len(auth.pubkey))

	// Generate MLDSA-87 ID Signature
	cdata := challenge.ChallengeData
	idsig, err := cryptod.SignMLDsa87(c.privkey, cdata)
	if err != nil {
	...
	// Add our record to response if it's newer than what the remote side has.
	ln := c.localnode.Node()
...
func (c *Codec) decodeHandshake(fromAddr string, head *Header) (n *enode.Node, auth handshakeAuthData, s *session, err error) {
	...
	// Decode the ephemeral public key (MLDSA-87 compatible)
	ephkey, err := cryptod.UnmarshalPubkey(auth.pubkey)
	if err != nil {
		return nil, auth, nil, errInvalidAuthKey
	}

	// Derive session keys using MLDSA-87 key types
