...
func (n *NodeConfig) MarshalJSON() ([]byte, error) {
	...
	if n.PrivateKey != nil {
		confJSON.PrivateKey = hex.EncodeToString(cryptod.FromMLDsa87(n.PrivateKey))
	}
...
func (n *NodeConfig) UnmarshalJSON(data []byte) error {
	...
	if confJSON.PrivateKey != "" {
		...
		privKey, err := cryptod.ToMLDsa87(key)
		if err != nil {
			return err
		}
...
func RandomNodeConfig() *NodeConfig {
	...
	prvkey, err := cryptod.GenerateMLDsa87Key()
	if err != nil {
		panic("unable to generate ML-DSA-87 key")
	}

	// Extract ML-DSA-87 Public Key from the Private Key
	pubKey := prvkey.Public().(*cryptod.PublicKey) // Correctly extract the public key
	...
	// Convert ML-DSA-87 Public Key to Enode ID
	enodId := enode.PubkeyToIDV4(pubKey)
