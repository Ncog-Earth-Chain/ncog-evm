...
type Config struct {
	...
	PrivateKey *cryptod.PrivateKey `toml:"-"`
...
type Server struct {
	...
	newTransport func(net.Conn, *cryptod.PublicKey) transport
...
type transport interface {
	// The two handshakes.
	doEncHandshake(prv *cryptod.PrivateKey) (*cryptod.PublicKey, error)
...
func (srv *Server) Self() *enode.Node {
	...
	if ln == nil {
		// Extract the public key from ML-DSA-87 private key
		pubKey := srv.PrivateKey.Public().(*cryptod.PublicKey)

		// Create an ENR (Ethereum Node Record) for the local node
		return enode.NewV4(pubKey, net.ParseIP("0.0.0.0"), 0, 0)
	}
	return ln.Node()
}
...
func (srv *Server) setupLocalNode() error {
	// Create the devp2p handshake.
	pubkey := cryptod.FromMLDsa87Pub(srv.PrivateKey.Public().(*cryptod.PublicKey))
...
old
	...
	func (srv *Server) setupConn(c *conn, flags connFlag, dialDest *enode.Node) error {
		fmt.Println("testing setupConn", "test")
...
func (srv *Server) setupConn(c *conn, flags connFlag, dialDest *enode.Node) error {
	...
	// If dialing, figure out the remote public key.
	var dialPubkey *cryptod.PublicKey
	if dialDest != nil {
		dialPubkey = new(cryptod.PublicKey)
		if err := dialDest.Load((*enode.MLDsa87Key)(dialPubkey)); err != nil {
			err = errors.New("dial destination doesn't have an ML-DSA-87 public key")
			srv.log.Trace("Setting up connection failed", "addr", c.fd.RemoteAddr(), "conn", c.flags, "err", err)
			return err
		}
	}
	...
	if id := c.node.ID(); !bytes.Equal(cryptod.Keccak512(phs.ID), id[:]) {
		clog.Trace("Wrong devp2p handshake identity", "phsid", hex.EncodeToString(phs.ID))
		return DiscUnexpectedIdentity
	}
	c.caps, c.name = phs.Caps, phs.Name
...
func nodeFromConn(pubkey *cryptod.PublicKey, conn net.Conn) *enode.Node {
	var ip net.IP
	var port int
	if tcp, ok := conn.RemoteAddr().(*net.TCPAddr); ok {
		ip = tcp.IP
