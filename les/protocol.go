...
// sign adds a signature to the block announcement using ML-DSA-87
func (a *announceData) sign(privKey *mldsa87.PrivateKey) {
	// Encode block info into RLP format
	rlpData, _ := rlp.EncodeToBytes(blockInfo{a.Hash, a.Number, a.Td})

	// Compute Keccak512 hash
	hash := cryptod.Keccak512(rlpData)

	// Sign the Keccak512 hash using ML-DSA-87
	sig, err := cryptod.SignMLDsa87(privKey, hash)
	if err != nil {
		fmt.Printf("Failed to sign block announcement: %v\n", err)
		return
	}

	// Store the signature in the announcement
	a.Update = a.Update.add("sign", sig)
}

// checkSignature verifies if the block announcement has a valid signature using ML-DSA-87
func (a *announceData) checkSignature(id enode.ID, update keyValueMap) error {
	...
	//  Extract the public key from `enode.ID`
	pubKeyBytes, err := id.MarshalText()
	if err != nil {
		return fmt.Errorf("failed to extract public key from enode ID: %v", err)
	}

	// Convert to ML-DSA-87 Public Key
	pubKey, err := cryptod.UnmarshalPubkey(pubKeyBytes)
	if err != nil {
		return fmt.Errorf("failed to convert enode ID to ML-DSA-87 key: %v", err)
	}

	// Verify the signature using ML-DSA-87
	isValid := cryptod.ValidateMLDsa87Signature(pubKey, hash, sig)
	if !isValid {
