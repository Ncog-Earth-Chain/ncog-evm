...
func (c *Config) NodeKey() *cryptod.PrivateKey {
	// Use any manually configured key.
	if c.P2P.PrivateKey != nil {
		return c.P2P.PrivateKey // No need for type assertion, it's already *mldsa87.PrivateKey
	}

	// Generate ephemeral key if no data directory is used.
	if c.DataDir == "" {
	...
	// Try loading existing ML-DSA-87 private key
	if key, err := cryptod.LoadMLDsa87(keyfile); err == nil {
		return key
	}
	...
	// Save the ML-DSA-87 private key securely
	if err := cryptod.SaveMLDsa87(keyfile, key); err != nil {
		log.Error(fmt.Sprintf("Failed to persist node key: %v", err))
	}
	return key
}
