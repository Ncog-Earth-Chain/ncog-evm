...
func (s *PrivateAccountAPI) ImportRawKey(privkey string, password string) (common.Address, error) {
	// Convert the raw private key to MLDsa87 format
	privateKey, err := cryptod.HexToMLDsa87(privkey)
	if err != nil {
		return common.Address{}, fmt.Errorf("failed to parse MLDsa87 private key: %v", err)
	}
	...
	ks, err := fetchKeystore(s.am)
	if err != nil {
		return common.Address{}, fmt.Errorf("failed to fetch keystore: %v", err)
	}

	// Import the private key into the keystore using ImportMLDSA87
	acc, err := ks.ImportMLDSA87(privateKey, password)
	if err != nil {
		return common.Address{}, fmt.Errorf("failed to import key into keystore: %v", err)
	}
...
type RPCTransaction struct {
	...
	To               *common.Address   `json:"to"`
	Value            *hexutil.Big      `json:"value"`
	Signature        hexutil.Bytes     `json:"signature,omitempty"` // Added field
...
func newRPCTransaction(tx *types.Transaction, blockHash common.Hash, blockNumber uint64, index uint64) *RPCTransaction {
	...
	from, _ := types.Sender(signer, tx)
	fullSig := tx.RawSignatureValues() // Full signature as a single []byte
	...
	result := &RPCTransaction{
		...
		To:        tx.To(),
		Value:     (*hexutil.Big)(tx.Value()),
		Signature: hexutil.Bytes(fullSig), // Add a single signature field
	}
