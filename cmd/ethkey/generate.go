...
`,
	...
	Action: func(ctx *cli.Context) error {
		...
		/* var privateKey *cryptod.PrivateKey
		var err error
		if file := ctx.String("privatekey"); file != "" {
			// Load private key from file.
			privateKey, err = cryptod.LoadMLDsa87(file)
			if err != nil {
				utils.Fatalf("Can't load private key: %v", err)
			}
		} else {
		...
		address := cryptod.PubkeyToAddress(*privateKey.Public().(*cryptod.PublicKey))
		...
		keyjson, err := keystore.EncryptKey(privateKey, passphrase, scryptN, scryptP)
		if err != nil {
			utils.Fatalf("Error encrypting key: %v", err)
		}

		// Store the encrypted key to disk.
		if err := os.MkdirAll(filepath.Dir(keyfilepath), 0700); err != nil {
			utils.Fatalf("Could not create directory %s", filepath.Dir(keyfilepath))
		}
		if err := ioutil.WriteFile(keyfilepath, keyjson, 0600); err != nil {
		...
		// Wrap the private key in a keystore.Key
		UUID, err := uuid.NewRandom()
		if err != nil {
			utils.Fatalf("Failed to generate random uuid: %v", err)
		}
		...
		key := &keystore.Key{
			Id:         UUID,
			Address:    address,
			PrivateKey: privateKey, // This requires *cryptod.PrivateKey support in keystore.Key
		}
