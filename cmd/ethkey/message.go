...
`,
	...
	Action: func(ctx *cli.Context) error {
		...
		privKey := key.PrivateKey // key.PrivateKey should already be of type *mldsa87.PrivateKey
		if privKey == nil {
			utils.Fatalf("Error: private key is nil")
		}

		// Use the cryptod package to sign the message
		signature, err := cryptod.SignMLDsa87(privKey, cryptod.Keccak512(message))
		if err != nil {
...
It is possible to refer to a file containing the message.`,
	...
	Action: func(ctx *cli.Context) error {
		...
		pubKey, err := cryptod.RecoverPubkey(cryptod.Keccak512(message), signature)
		if err != nil {
			utils.Fatalf("Signature verification failed: %v", err)
		}

		pubKeyBytes := cryptod.FromMLDsa87Pub(pubKey)
		recoveredAddress := cryptod.PubkeyToAddress(*pubKey)
		success := address == recoveredAddress

		out := outputVerify{
			Success:            success,
			RecoveredPublicKey: hex.EncodeToString(pubKeyBytes),
