...
func (t *txWithKey) UnmarshalJSON(input []byte) error {
	...
	if key.Key != nil {
		...
		if mldsaKey, err := cryptod.HexToMLDsa87(k); err != nil {
			return err
		} else {
			t.key = mldsaKey
		}
...
/* func signUnsignedTransactions(txs []*txWithKey, signer types.Signer) (types.Transactions, error) {
	...
	for i, txWithKey := range txs {
		...
		if key != nil && len(sig) == 0 {
			// Convert key to *cryptod.PrivateKey
			cryptodKey := (*cryptod.PrivateKey)(key)

			// Sign the transaction
			signed, err := types.SignTx(tx, signer, cryptodKey)
			if err != nil {
				return nil, NewError(ErrorJson, fmt.Errorf("tx %d: failed to sign tx: %v", i, err))
			}
			signedTxs = append(signedTxs, signed)
...
func signUnsignedTransactions(txs []*txWithKey, signer types.Signer) (types.Transactions, error) {
	...
	for i, txWithKey := range txs {
		tx := txWithKey.tx
		key := txWithKey.key           // This should be of type *mldsa87.PrivateKey
		sig := tx.RawSignatureValues() // Full signature as []byte
		...
		if key != nil && len(sig) == 0 {
			...
			signed, err := types.SignTx(tx, signer, key)
			if err != nil {
				return nil, fmt.Errorf("tx %d: failed to sign tx: %w", i, err)
			}
			signedTxs = append(signedTxs, signed)
		} else {
			// Already signed
