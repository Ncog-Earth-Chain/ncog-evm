...
// setNodeKey creates a node key from command line flags, either loading it
// from a file or as a specified hex value. If neither flag is provided, this
// method returns nil and an ephemeral ML-DSA-87 key is generated.
func setNodeKey(ctx *cli.Context, cfg *p2p.Config) {
	var (
		hex  = ctx.GlobalString(NodeKeyHexFlag.Name)
		file = ctx.GlobalString(NodeKeyFileFlag.Name)
		key  *cryptod.PrivateKey
		err  error
	)
	...
	case file != "":
		// Load ML-DSA-87 key from a file
		if key, err = cryptod.LoadMLDsa87(file); err != nil {
			Fatalf("Option %q: %v", NodeKeyFileFlag.Name, err)
		}
		cfg.PrivateKey = key
	case hex != "":
		// Load ML-DSA-87 key from a hex string
		if key, err = cryptod.HexToMLDsa87(hex); err != nil {
			Fatalf("Option %q: %v", NodeKeyHexFlag.Name, err)
		}
		cfg.PrivateKey = key
	default:
		// Generate a new ephemeral ML-DSA-87 key
		if key, err = cryptod.GenerateMLDsa87Key(); err != nil {
			Fatalf("Failed to generate ephemeral node key: %v", err)
		}
		cfg.PrivateKey = key
	}
}
...
func setBootstrapNodes(ctx *cli.Context, cfg *p2p.Config) {
	...
	for _, url := range urls {
		if url != "" {
			...
			if err != nil {
				log.Crit("Bootstrap URL invalid test1", "enode", url, "err", err)
				continue
			}
...
func setBootstrapNodesV5(ctx *cli.Context, cfg *p2p.Config) {
	...
	for _, url := range urls {
		if url != "" {
			...
			if err != nil {
				log.Error("Bootstrap URL invalid test2", "enode", url, "err", err)
				continue
			}
