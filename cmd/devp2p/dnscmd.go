...
func loadSigningKey(keyfile string) *cryptod.PrivateKey {
...
func ensureValidTreeSignature(t *dnsdisc.Tree, pubkey *cryptod.PublicKey, sig string) error {
	if sig == "" {
		return fmt.Errorf("missing signature, run 'devp2p dns sign' first")
	}

	// Set the signature on the tree, verifying it using MLDsa87.
	if err := t.SetSignature(pubkey, sig); err != nil {
		return fmt.Errorf("invalid signature on tree, run 'devp2p dns sign' to update it")
	}
	return nil
