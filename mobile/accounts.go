...
// ImportMLDSA87Key imports an MLDSA87 private key.
func (ks *KeyStore) ImportMLDSA87Key(key []byte, passphrase string) (account *Account, _ error) {
	privKey, err := cryptod.HexToMLDsa87(string(key))
	if err != nil {
		return nil, err
	}
	acc, err := ks.keystore.ImportMLDSA87(privKey, passphrase)
	if err != nil {
		return nil, err
	}
	return &Account{acc}, nil
...
*/
// ImportPreSaleKey decrypts the given presale wallet and stores a key file.
