...
/* type TxData interface {
	...
	setSignatureValues(chainID *big.Int, fullSig []byte)
} */

type TxData interface {
	txType() byte      // returns the type ID
	copy() TxData      // creates a deep copy and initializes all fields
	chainID() *big.Int // retrieves the chain ID
	accessList() AccessList
	data() []byte
	gas() uint64
	gasPrice() *big.Int
...
func sanityCheckSignature(signature []byte, messageHash []byte, pubKey *cryptod.PublicKey) error {
	// Validate the length of the signature
	if len(signature) != 4627 { // Ensure the signature length matches ML-DSA-87
		return ErrInvalidSig
	}

	// Use the cryptod package to verify the signature
	valid := cryptod.ValidateMLDsa87Signature(pubKey, messageHash, signature)
	if !valid {
		return ErrInvalidSig
	}
...
func isProtectedV(V *big.Int) bool {
	if V == nil {
		return true // Assume protected if V is absent
	}
	if V.BitLen() <= 8 {
		v := V.Uint64()
		return v != 27 && v != 28 && v != 1 && v != 0
...
func (tx *Transaction) Protected() bool {
	switch tx := tx.inner.(type) {
	case *LegacyTx:
		// For ML-DSA-87, assume all transactions are protected
		if tx.Signature != nil {
			return true
		}
		return false
...
func (tx *Transaction) WithSignature(signer Signer, sig []byte) (*Transaction, error) {
	...
	cpy := tx.inner.copy()
	cpy.setSignatureValues(signer.ChainID(), fullSig)
